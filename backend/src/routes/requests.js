...
router.post("/", async (req, res) => {
  try {
    ...
    if (!student_id || !tutor_id)
      return res.status(400).json({ error: "student_id and tutor_id are required" });

    // Validar existencia de student y tutor
    ...
    if (!tutorRows.length) return res.status(404).json({ error: "Tutor not found" });

    // Insertar request en MySQL
...
// GET /requests
// -------------------------
// En requests.js - modifica el endpoint GET /
...
// -------------------------
// En requests.js - Modifica el endpoint PUT /:id
router.put("/:id", async (req, res) => {
  try {
    ...
    // Obtener la request y validar tutor
    const [rows] = await pool.query(
      `SELECT r.*, 
              s.id AS student_db_id,          -- ← Cambiado: id de students
              t.id AS tutor_db_id,            -- ← Cambiado: id de tutors
    ...
    if (!rows.length)
      return res.status(404).json({ error: "Request not found or tutor mismatch" });
    ...
    res.json({ 
      ...
      student_db_id: rows[0].student_db_id,      // ← Nuevo: id de tabla students
      tutor_db_id: rows[0].tutor_db_id,          // ← Nuevo: id de tabla tutors
      student_user_id: rows[0].student_user_id,  // ← Mantener por si acaso
      tutor_user_id: rows[0].tutor_user_id,      // ← Mantener por si acaso
