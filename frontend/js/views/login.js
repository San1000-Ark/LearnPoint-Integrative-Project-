...
export function initLogin(navigate) {
  ...
  loginForm.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      // Validamos que el rol coincida con el botón seleccionado
      if (data.role !== userType) {
        throw new Error(`This user is registered as ${data.role}, not ${userType}`);
      }

    localStorage.setItem("lp_userId", data.user.id); // users.id
    if (data.role === "student") localStorage.setItem("lp_studentId", data.user.studentId);
    if (data.role === "tutor")   localStorage.setItem("lp_tutorId", data.user.tutorId);
